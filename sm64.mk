...
# I know this breaks the convention. I'll fix it  -iProgram
ifeq ($(TARGET_NANOSHELL), 1)
	TARGET = sm64.$(VERSION).nse
endif
...
ifeq ($(TARGET_NANOSHELL),1)
  SRC_DIRS += src/nanoshell src/nanoshell/crapmath src/nanoshell/tinygl
endif
...
#  -mno-x87                \
  ...
  CC  := clang   -Isrc/nanoshell/include -I../../crt/include -DNANOSHELL -ffreestanding -target i686-elf -nostdinc -fno-exceptions -Wno-unused-function $(CPU_FEATURE_FLAGS)
  CXX := clang++ -Isrc/nanoshell/include -I../../crt/include -DNANOSHELL -ffreestanding -target i686-elf -nostdinc -fno-exceptions -Wno-unused-function $(CPU_FEATURE_FLAGS)
else
  CC := gcc
  CXX := g++
endif
...
ifeq ($(TARGET_NANOSHELL),1)
  # Hack for now - directly include the libc from a different program
  LD := ld
  NANOSHELL_LDFLAGS += ../../tools/libgcc-i686.a ../../crt/lib/libnanoshell.a ../../crt/lib/crt1.o
endif
OBJDUMP := objdump
OBJCOPY := objcopy
PYTHON := python3
...
ifeq ($(TARGET_NANOSHELL),1)
  GFX_CFLAGS += -DENABLE_WM_NANOSHELL
endif
...
$(BUILD_DIR)/%.o: %.cpp
	...
	$(V)$(CXX) -fsyntax-only $(CFLAGS) -MMD -MP -MT $@ -MF $(BUILD_DIR)/$*.d $<
	$(V)$(CXX) -c $(CFLAGS) -o $@ $<
$(BUILD_DIR)/%.o: %.c
	$(call print,Compiling:,$<,$@)
