...
**NOTE:** You MUST check it out in `<NanoShell root folder>/apps/sm64`! Otherwise, this won't be able to find NanoShell specific includes.
...
5. The executable binary will be located at `build/<VERSION>_pc/sm64.<VERSION>.nse` or `sm64.nse`.

Additionally, since this integrates well into the NanoShell build system, you can just `./tryapp.sh sm64` from NanoShell's root.

**NOTE:** Currently builds for other platforms are broken, because it'll always build for NanoShell. This may be fixed in the future.
