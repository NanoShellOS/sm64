	...
	const void * indices)
{
	GLsizei i;
	GLint element;
	const uint8_t *u8ptr = indices;
	const uint16_t *u16ptr = indices;
	const uint32_t *u32ptr = indices;
...
#include "error_check_no_context.h"
	...
	{
		switch (type) {
			case GL_UNSIGNED_BYTE:  element = *u8ptr++;  break;
			case GL_UNSIGNED_SHORT: element = *u16ptr++; break;
			case GL_UNSIGNED_INT:   element = *u32ptr++; break;
			default: {
				#define ERROR_FLAG GL_INVALID_ENUM
				#include "error_check_no_context.h"
			}
